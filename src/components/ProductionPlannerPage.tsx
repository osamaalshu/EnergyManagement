...
const ProductionPlannerPage: FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  return (
    <section className="space-y-6">
      ...
      {/* TRACK B — plant-state telemetry (simulated digital twin) */}
      {productionData.plant && (() => {
        const pl = productionData.plant; const r = pl.real; const e = r.energy_by_state_kwh; const tot = r.energy_kwh || 1;
        const series = Object.values(pl.trace.reduce((acc, row) => { (acc[row.hour] ??= { hour: Math.round(row.hour), kw: 0 }).kw += row.kw; return acc; }, {} as Record<number, { hour: number; kw: number }>));
        const STATE_BG: Record<string, string> = { run: 'bg-accent', setup: 'bg-amber-500', idle: 'bg-slate-300 dark:bg-white/20', down: 'bg-rose-500' };
        return (
          <div className="card-surface p-5">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <h3 className="text-sm font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">Plant telemetry — simulated digital twin (Track B)</h3>
              <span className="rounded-full bg-amber-50 px-2 py-0.5 text-[0.6rem] font-semibold uppercase text-amber-700 dark:bg-amber-500/10 dark:text-amber-300">simulated</span>
            </div>
            <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
              <div className="rounded-lg border border-slate-200/70 p-3 dark:border-white/10"><p className="text-[11px] uppercase tracking-wide text-slate-400">Availability</p><p className="mt-0.5 font-mono text-lg font-semibold text-slate-900 dark:text-white">{Math.round(r.availability * 100)}%</p></div>
              <div className="rounded-lg border border-slate-200/70 p-3 dark:border-white/10"><p className="text-[11px] uppercase tracking-wide text-slate-400">Downtime</p><p className="mt-0.5 font-mono text-lg font-semibold text-rose-600 dark:text-rose-400">{num(r.downtime_h)} h</p></div>
              <div className="rounded-lg border border-slate-200/70 p-3 dark:border-white/10"><p className="text-[11px] uppercase tracking-wide text-slate-400">Energy</p><p className="mt-0.5 font-mono text-lg font-semibold text-amber-600 dark:text-amber-400">{num(r.energy_kwh)} kWh</p></div>
              <div className="rounded-lg border border-slate-200/70 p-3 dark:border-white/10"><p className="text-[11px] uppercase tracking-wide text-slate-400">Energy / unit</p><p className="mt-0.5 font-mono text-lg font-semibold text-slate-900 dark:text-white">{r.kwh_per_unit} kWh</p></div>
            </div>
            <p className="mt-4 mb-1 text-[11px] uppercase tracking-wide text-slate-400">Energy by machine state</p>
            ...
            <div className="mt-1 flex flex-wrap gap-3 text-[11px] text-slate-500 dark:text-slate-400">
              {(['run', 'setup', 'idle', 'down'] as const).map((s) => (<span key={s} className="inline-flex items-center gap-1"><span className={`h-2 w-2 rounded-sm ${STATE_BG[s]}`} />{s} {num(e[s] || 0)}</span>))}
            </div>
            <div className="mt-4 h-32">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={series} margin={{ top: 4, right: 8, left: 0, bottom: 4 }}>
                  <XAxis dataKey="hour" tick={{ fill: 'var(--muted-text)', fontSize: 10 }} tickLine={false} axisLine={{ stroke: 'var(--grid-stroke)' }} label={{ value: 'hour', position: 'insideBottom', offset: -2, fill: 'var(--muted-text)', fontSize: 10 }} />
                  <YAxis tick={{ fill: 'var(--muted-text)', fontSize: 10 }} tickLine={false} axisLine={false} width={28} />
            ...
            <p className="mt-1 text-[11px] text-slate-400">Power drawn by machine state over the run, with stochastic breakdowns (availability {Math.round(r.availability * 100)}% vs {Math.round(pl.ideal.availability * 100)}% ideal). Generated by the SimPy plant engine — wire the line's sensors and this becomes the live trace, calibrating downtime &amp; energy to reality.</p>
          </div>
        );
      })()}
